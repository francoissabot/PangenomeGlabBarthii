...
\begin{flushleft}
{\Large
\textbf\newline{Comparison of two African rice species through \\a new pan-genomic approach on massive data}
}
\newline
% authors go here:
\\
...
%  \subsection*{Assembly and filtering}
  %Unproperly mapped in pair reads (\textit{i.e.} without the 0x02 flag in BAM files) were assembled using Abyss \cite{Simpson2009}, under standard conditions. Duplicated assembled contigs were removed with dedupe2 from the BBtools suite \url{http://jgi.doe.gov/data-and-tools/bbtools/} applying a threshold of 80\% of identity. Three methods of deduplications were tested: (\textit{i}) deduplication within all individuals before aggregation and deduplication of the total data; (\textit{ii}) aggregation of the total data then complete deduplication; (\textit{iii}) one-by-one aggregation and deduplication. The three methods provided almost the same results (difference lower than 2\%, data not shown), and we decided to focus on the last method. Individual reads were then re-mapped as detailed above for following analyses.
...
%Reverifier avec les nouvelles données
%Unproperly and non-mapped reads were then extracted, re-assembled and deduplicated for each species, and the reads were then back mapped to this dispensable-genome data. As a first results, we were able to re-assemble almost 38 Mb per individual (whatever is the species), representing after deduplication 1 Gb and 1.2Gb of non-reference sequences for \textit{O.~glaberrima} and \textit{O.~barthii} (See the\texttt{Table~\ref{statAssembly}} for more details).
%The new sequence data were then subject to automatic annotation using MAKER-P (!!REF!!), using the same parameters and data used in \cite{Monat2017}. %rajouter la ref GBE - on pourrait essayer de refaire tourner avec les RNAseq d'arcad aussi
